...
export class Highlighter {
  ...
  public async applyDimBackground(
    ...
    // Don't return early - we need to dim even if activeItems is empty
    // because we want to dim everything when there are no active items
    ...
    // Process all models in parallel to ensure dimming completes faster
    const dimmingPromises: Promise<void>[] = [];
    ...
    for (const modelId of this.modelManager.modelIds) {
      ...
      const dimPromise = (async () => {
        ...
        if (!allLocalIds || allLocalIds.length === 0) return;
      ...
      dimmingPromises.push(dimPromise);
    }

    // Wait for all dimming operations to complete before proceeding
    await Promise.all(dimmingPromises);
...
/*           console.log(
  ...
  public resetHighlight(): void {
    // Reset highlighting for ALL models/fragments, not just tracked ones
    // This ensures dimming from previous operations is also cleared
    for (const modelId of this.modelManager.modelIds) {
      const model = this.modelManager.fragmentManager.list.get(modelId);
      if (!model) continue;
      try {
