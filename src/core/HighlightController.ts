...
export class HighlightController {
  ...
  private async applyHighlightsFromToggles(): Promise<void> {
    ...
    try {
      ...
      // Always apply dimming when we have active highlighting - applyDimBackground
      // handles empty maps correctly by dimming all elements
      try {
        const activeItems = await this.getCombinedItemsForGroups(activeGroups);
        // Always apply dimming when we have active highlighting - applyDimBackground
        // handles empty maps correctly by dimming all elements
        // IMPORTANT: Wait for dimming to fully complete before applying highlights
      ...
      // 4) Update status indicator after all operations complete
      this.updateStatusIndicator(`Highlighting: ${parts.join(" + ")}`);
    } finally {
      // 5) End batch mode and render everything together (dimming + highlights)
      this.highlighter.endBatch();
      this.hideApplyingOverlay();
    }
  }
