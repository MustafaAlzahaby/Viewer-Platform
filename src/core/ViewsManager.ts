...
/*       console.log(
  ...
  private async createViewForSpatialElement(element: any, fragment: any): Promise<void> {
    try {
      ...
      // @ts-expect-error - views.create accepts object parameter at runtime
...
/*         console.log(`... and ${meshCount - 20} more meshes (see "meshDetails" array)`);
      ...
      // Group meshes by IFC type (commented out but kept for potential future use)
  ...
  public dispose(): void {
    // Cleanup method for ViewsManager
    if (this.panel) {
      this.panel.remove();
      this.panel = null;
    }
